...
## 原子操作判据（atomic 什么时候必须用、什么时候别用）

> 来源：cosnet `Socket.status` 的 `-race` 实报（读写循环双 defer 并发 `disconnect`）。
> 修复时把该字段 13 处非原子访问全部收口，随后复盘出这套判据。**原则一句话：
> 竞争按后果分级处理，不是见 race 就原子化。**

### 分级：三类访问，三种处置

| 级别 | 特征 | 处置 |
|------|------|------|
| 🔴 必须修 | **无同步的并发写**，以及**与写并发的读** | 写 `atomic.Store`/CAS，读 `atomic.Load`，没有商量 |
| 通常也修 | 良性竞争：读旧值只多一拍判断窗口（KeepAlive/CanWrite 类探询） | `atomic.Load` 在 x86 就是一条 `MOV`，零成本保险，默认做；实测有代价的热路径可保留普通读并**注释声明良性竞争** |
| 不用修 | 已有 happens-before：goroutine 启动前的初始化写、锁内访问、channel 收发序 | 改了不算错但无必要（cosnet `connect()` 的初始化写即属此类，协程在其后才 spawn） |

🔴 级别为什么没有商量余地：数据竞争在 Go 内存模型里是**未定义行为**——不是说现在就会撕裂
（对齐 int32 实际不会），而是编译器有权做寄存器缓存等优化，且今天无害的竞争，明天有人在
旁边加一段逻辑就可能变成真故障。`-race` 报出来的每一对至少有一侧要动。

### atomic 解决不了的（比"改不改"更重要）

1. **check-then-act / TOCTOU**：原子读只消竞争，**不消逻辑窗口**。`if status==Connected { 清心跳 }`
   原子化后照样有窗口。语义正确性靠 **CAS 收口状态转换**（cosnet 的 `disconnect`/`Close`
   入口 CAS 才是真正仲裁"谁有资格继续"的那道闸），读侧原子化只是让整个字段在内存模型上站得住。
2. **多字段一致性**：状态由两个以上字段表达时，单字段 atomic 救不了，要锁或版本号。
3. **已有锁保护的字段**：再叠 atomic 是制造两套同步语义，倒退（本框架 `Player` 的 values
   走玩家锁，就是"不要 atomic"的正面例子；`p.Status` 在锁外才必须是 CAS 状态机）。
4. **int64/arm64 对齐**：确认对齐或直接用 `atomic.Int64` 类型。

### 判据：五问，按顺序

1. 字段有并发写吗？→ 有：写侧必须原子/锁收口，进下一问；没有并发写（只读或初始化后不变）→ 结束。
2. 有与写并发的读吗？→ 有：读侧 `atomic.Load`（零成本，默认做）。
3. 读到旧值有**业务后果**吗？→ 有：这不是 atomic 的问题，是 TOCTOU——用 CAS 收口转换或加锁。
4. 字段已被某把锁覆盖？→ 是：**不要** atomic，访问统一走那把锁。
5. 需要多个字段保持一致？→ 是：锁/版本号，atomic 不够。

### `-race` 的边界

`-race` 只证明存在、不证明不存在——跑干净 ≠ 无竞争，只说明这套测试没触发。
所以"整字段收口"的价值不是修 bug，而是**关掉一整个 UB 类别**，不赌测试覆盖。
反过来，CI 里挂 `-race` 跑全量测试是这套技术栈的默认要求（Windows 需 CGO+gcc，
gcc 不在 PATH 时先 `gcc --version` 确认）。
...
`c.Send` 在没有 Player 时靠请求 metadata 里的 socketId/UID 定位推送目标——**tick 协程手里根本没有请求**，
这条路从原理上就覆盖不到（日志只有一句"SocketId/UID 均为空"，服务端一切正常、客户端什么都收不到）。

所以：**取玩家信息的那一次（RPC 拉档案 / 持锁那一刻）就把网关地址 + 会话标识一起取回来自己存着**，
之后按它直接投网关。踩过两次，症状都是"服务端日志全对、客户端一片空白"。

推论——**推送本身不属于"玩家数据"**：网关只要「网关地址 + socketId/UID」就能投到那条连接上
（定位优先级 socketId > UID：请求驱动的推送认 socketId，主动推送按 UID 直查会话表；
**GUID 只做认证、不参与定位**）。不需要玩家对象。这让"不碰玩家数据的服"也能自己推消息，
不必绕一趟回主服代发。

> ⚠ **认死连接的那个（socketId）必须取当次请求的活值，不能用早先存下的快照**——顶号/重连之后，
> 上一代的数据会被推给刚上来的另一个人。UID 是账号级定位，用快照只会投给"当前在线的那个角色"。
